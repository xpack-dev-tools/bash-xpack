...
[![npm (scoped)](https://img.shields.io/npm/v/%40xpack-dev-tools%2Fbash?label=xpm&color=blue)](https://www.npmjs.com/package/@xpack-dev-tools/bash/)
![GitHub Downloads (all assets, all releases)](https://img.shields.io/github/downloads/xpack-dev-tools/bash-xpack/total)
[![npm](https://img.shields.io/npm/dm/%40xpack-dev-tools%2Fbash?label=xpm%20downloads)](https://www.npmjs.com/package/@xpack-dev-tools/bash/)

The **xPack GNU Bash** can be installed automatically, using the `xpm` command
(the recommended method), or manually, by downloading and unpacking one of the
platform specific archives.

## Automated install

The easiest (and recommended) way to install GNU Bash
is via **xpm** and the package available
as <a href={`https://www.npmjs.com/package/@xpack-dev-tools/bash`}><code>@xpack-dev-tools/bash</code></a> from
...
<code>xpacks/@xpack-dev-tools/bash</code>.
...
**bash** package into the project.

The command to install the **latest** available version of **bash** is:
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/bash@${customField('releaseVersion')} --verbose
`} </CodeBlock>
...
* identify the platform specific archive for the latest available version
  of GNU Bash, download it into a cache and unpack
...
* create a local symbolic link
  like <code>xpacks/@xpack-dev-tools/bash</code> pointing to the
...
* add links/forwarders into
  ...
  <code>xpacks/@xpack-dev-tools/bash/.content/bin</code>;
* add <code>@xpack-dev-tools/bash</code> to
  `package.json` as a
  development dependency; this associates a specific version of GNU Bash with
...
it is also possible to install GNU Bash only globally in
...
<CodeBlock language="sh"> {
`xpm install @xpack-dev-tools/bash@latest --global --verbose
`} </CodeBlock>
...
In order to access the GNU Bash binaries installed in
...
<CodeBlock language="sh"> {
`export PATH=$HOME/AppData/Roaming/xPacks/\@pack-dev-tools/bash/${customField('xpackVersion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/Library/xPacks/\@pack-dev-tools/bash/${customField('xpackVersion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`export PATH=$HOME/.local/xPacks/@xpack-dev-tools/bash/${customField('xpackVersion')}.${customField('npmSubversion')}/.content/bin:$PATH
`} </CodeBlock>
...
<CodeBlock language="sh"> {
`xpm uninstall @xpack-dev-tools/bash --verbose
`} </CodeBlock>

To completely remove the package from the user global xPacks store:

<CodeBlock language="sh"> {
`xpm uninstall --global @xpack-dev-tools/bash --verbose
`} </CodeBlock>
...
For all platforms, the **xPack GNU Bash** binaries are released as portable
archives that can be installed in any location.

The archives can be downloaded from
the <a href={`https://github.com/xpack-dev-tools/bash-xpack/releases`}>GitHub Releases</a> pages.
...
The Windows versions of **xPack GNU Bash**
are packed as `.zip` files.
Download the latest version named like:

<ul>
<li><code>xpack-bash-{customField('xpackVersion')}-win32-x64.zip</code></li>
</ul>
...
the <code>%USERPROFILE%\\AppData\\Roaming\\xPacks\\bash</code> folder, for
example <code>C:\\Users\\ilg\\AppData\\Roaming\\xPacks\\bash\\xpack-bash-{customField('xpackVersion')}</code>.
...
<ul>
<li><code>xpack-bash-{customField('xpackVersion')}-darwin-x64.tar.gz</code></li>
<li><code>xpack-bash-{customField('xpackVersion')}-darwin-arm64.tar.gz</code></li>
</ul>
...
example <code>/Users/ilg/Library/xPacks/bash/xpack-bash-{customField('xpackVersion')}</code>:

<CodeBlock language="console"> {
`mkdir -p ~/Library/xPacks/bash
cd ~/Library/xPacks/bash

tar xvf ~/Downloads/xpack-bash-${customField('xpackVersion')}-darwin-x64.tar.gz
chmod -R -w xpack-bash-${customField('xpackVersion')}
`} </CodeBlock>
...
The GNU/Linux versions of **xPack GNU Bash**
are packed as `.tar.gz` archives.
Download the latest version named like:

<ul>
<li><code>xpack-bash-{customField('xpackVersion')}-linux-x64.tar.gz</code></li>
<li><code>xpack-bash-{customField('xpackVersion')}-linux-arm64.tar.gz</code></li>
<li><code>xpack-bash-{customField('xpackVersion')}-linux-arm.tar.gz</code></li>
</ul>
...
The recommended location is
the <code>~/.local/xPacks/bash</code> folder, for
example <code>/home/ilg/.local/xPacks/bash/xpack-bash-{customField('xpackVersion')}</code>:

<CodeBlock language="console"> {
`mkdir -p ~/.local/xPacks/bash
cd ~/.local/xPacks/bash

tar xvf ~/Downloads/xpack-bash-${customField('xpackVersion')}-linux-x64.tar.gz
chmod -R -w xpack-bash-${customField('xpackVersion')}
`} </CodeBlock>
